export const tagIconMap = [
  { keywords: ['jewish'], icon: 'menorah', alt: 'Menorah' },
  { keywords: ['catholic'], icon: 'christmas-tree', alt: 'Christmas tree' },
  { keywords: ['non-religious'], icon: 'martini', alt: 'Martini' },
  {
    keywords: ['democrat', 'liberal', 'obama'],
    icon: 'democrat',
    alt: 'Donkey',
  },
  { keywords: ['lgbt'], icon: 'lgbt', alt: 'LGBT' },
  { keywords: ['actress', 'actor'], icon: 'camera', alt: 'Camera' },
  { keywords: ['years old'], icon: 'hourglass', alt: 'Hourglass' },
  { keywords: ['ancestry'], icon: 'seedling', alt: 'Seedling' },
];
